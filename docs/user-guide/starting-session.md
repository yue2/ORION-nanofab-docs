...
revision: "0.1"
nav_previous: "Sample Loading & Unloading"
nav_previous_path: "./user-guide/loading-unloading.md"
...
2. From the left column, find Sample List. Select 'Std Au', click 'go to' at the right bottom corner of the Sample List window to go to the standard gold sample. **Take note of the movement in the chamber scope. Click stop immediately when the tallest point of you sample is within 2 mm of the detector.**
3. From the top menu bar, click Power Systems to open the power system window. Power on the ET detector. From the left column, find ET detector tab, set brightness and contrast to 30% - 50% each.
...
6. From the left column, find GFIS settings, click 'Show all' to expand the options. 
    1. Confirm the appropriate aperture (Default is #2 10 um He) before turning on the gun. Choose the aperture type based on the gas you will use.
    ...
    5. Enable Lens 2 power, increase Lens 2 voltage gradually using the GUI slider in the software or the focus knob on keyboard. **Check the leakage current of Lens 2 when powering up. Do not exceed ==check the current==**
...
## Check trimer status
	...
	5. Confirm the tilt is corrected. Beam current should reach a maximum value at the correct tilt position, and the trimer atom appears at the center of the crosshair. If the beam tilt exeeds +- 200 mrad, the aperture housing needs to be manually corrected. Contact a superuser.
...
> If the beam tilt exeeds +- 200 mrad, the aperture housing needs to be manually corrected. Contact a superuser.
